...
namespace JoyLib.Code.World.Generators.Interiors
{
    public class DungeonCorridorGenerator
    {
        ...
        public DungeonCorridorGenerator(GeneratorTileType[,] tilesRef, List<RectInt> roomsRef, int bendRef, RNG roller)
        {
            ...
            this.m_Rooms = new List<InteriorRoom>();

            for (int i = 0; i < roomsRef.Count; i++)
            {
                this.m_Rooms.Add(new InteriorRoom(roomsRef[i], new List<InteriorRoom>()));
            }
        ...
        public GeneratorTileType[,] GenerateCorridors()
        {
            for (int a = 0; a < PASSES; a++)
            {
                ...
                {
                    for (int j = (int) this.m_Rooms[i].sizes.xMin; j < (int) this.m_Rooms[i].sizes.xMax; j++)
                    {
                        for (int k = (int) this.m_Rooms[i].sizes.yMin; k < (int) this.m_Rooms[i].sizes.yMax; k++)
                        {
