...
namespace InventoryMaster.Scripts.ExampleScript
{
    public class PlayerInventory : MonoBehaviour
    {
        ...
        private Inventory.Inventory craftSystemInventory;
        private CraftSystem.CraftSystem cS;
        private Inventory.Inventory mainInventory;
        private Inventory.Inventory characterSystemInventory;
        ...
        {
            Inventory.Inventory.ItemEquip += OnBackpack;
            Inventory.Inventory.UnEquipItem += UnEquipBackpack;

            Inventory.Inventory.ItemEquip += OnGearItem;
            Inventory.Inventory.ItemConsumed += OnConsumeItem;
            Inventory.Inventory.UnEquipItem += OnUnEquipItem;

            Inventory.Inventory.ItemEquip += EquipWeapon;
            Inventory.Inventory.UnEquipItem += UnEquipWeapon;
        }
        ...
        void EquipWeapon(Item.Item item)
        {
        ...
        void UnEquipWeapon(Item.Item item)
        {
            if (item.itemType == ItemType.Weapon)
            {
        ...
        void OnBackpack(Item.Item item)
        {
            if (item.itemType == ItemType.Backpack)
            {
                for (int i = 0; i < item.itemAttributes.Count; i++)
                {
                    if (mainInventory == null)
                        mainInventory = inventory.GetComponent<Inventory.Inventory>();
        ...
        void UnEquipBackpack(Item.Item item)
        {
            if (item.itemType == ItemType.Backpack)
        ...
        void Start()
        {
            ...
            if (characterSystem != null)
                characterSystemInventory = characterSystem.GetComponent<Inventory.Inventory>();
            if (craftSystem != null)
                craftSystemInventory = craftSystem.GetComponent<Inventory.Inventory>();
        }
        ...
        public void OnConsumeItem(Item.Item item)
        {
            for (int i = 0; i < item.itemAttributes.Count; i++)
            {
        ...
        public void OnGearItem(Item.Item item)
        {
            for (int i = 0; i < item.itemAttributes.Count; i++)
            {
                if (item.itemAttributes[i].attributeName == "Health")
        ...
        public void OnUnEquipItem(Item.Item item)
        {
            for (int i = 0; i < item.itemAttributes.Count; i++)
            {
                if (item.itemAttributes[i].attributeName == "Health")
