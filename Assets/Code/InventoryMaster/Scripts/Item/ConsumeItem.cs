...
using InventoryMaster.Scripts.ExampleScript;
    ...
    public class ConsumeItem : MonoBehaviour, IPointerDownHandler
    {
        ...
        void Start()
        {
            ...
            if (GameObject.FindGameObjectWithTag("Tooltip") != null)
                tooltip = GameObject.FindGameObjectWithTag("Tooltip").GetComponent<Tooltip.Tooltip>();
        ...
        public void OnPointerDown(PointerEventData data)
        {
            ...
            {
                bool gearable = false;
                Inventory.Inventory inventory = transform.parent.parent.parent.GetComponent<Inventory.Inventory>();
                ...
                if (data.button == PointerEventData.InputButton.Right)
                {
                    ...
                    {
                        bool check = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().inventory.GetComponent<Inventory.Inventory>().checkIfItemAllreadyExist(item.itemID, item.itemValue);

                        if (!check)
                        {
                            GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().inventory.GetComponent<Inventory.Inventory>().addItemToInventory(item.itemID, item.itemValue);
                            GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().inventory.GetComponent<Inventory.Inventory>().stackableSettings();
                        }
                        CraftSystem.CraftSystem cS = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().craftSystem.GetComponent<CraftSystem.CraftSystem>();
                        cS.deleteItems(item);
                        ...
                        gearable = true;
                        GameObject.FindGameObjectWithTag("MainInventory").GetComponent<Inventory.Inventory>().updateItemList();
                    }
                    else
                    {
                        bool stop = false;
                            ...
                            for (int i = 0; i < eS.slotsInTotal; i++)
                            {
                                if (itemTypeOfSlot[i].Equals(item.itemType))
                                {
                                    if (eS.transform.GetChild(1).GetChild(i).childCount == 0)
                                    {
                                        ...
                                        eS.gameObject.GetComponent<Inventory.Inventory>().updateItemList();
                                    ...
                                    if (itemTypeOfSlot[i].Equals(item.itemType))
                                    {
                                        if (eS.transform.GetChild(1).GetChild(i).childCount != 0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (this.gameObject.transform.parent.parent.parent.GetComponent<Hotbar.Hotbar>() != null)
        ...
        public void createDuplication(GameObject Item)
        {
            ...
            GameObject dup = mainInventory.GetComponent<Inventory.Inventory>().addItemToInventory(item.itemID, item.itemValue);
