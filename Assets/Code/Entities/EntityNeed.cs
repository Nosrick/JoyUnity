...
namespace JoyLib.Code.Entities
{
    public class EntityNeed
    {
            ...
            int happinessThreshold, int averageForDay, int averageForWeek, string name)
        {
            ...
            m_FilePath = Directory.GetCurrentDirectory() + "//Data//Scripts//Needs//" + this.name + ".py";
            try
            {
                //m_PythonObject = Scripting.PythonEngine.ExecuteClassFunction(m_FilePath, this.name, "ReturnSelf");
            }
            catch (Exception e)
            {
